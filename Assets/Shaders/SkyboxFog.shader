...
SubShader {
	...
	Pass {
		...
		v2f vert (appdata_t v)
		{
			...
			OUT.dir = mul(unity_ObjectToWorld, v.vertex);
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		// if y < 0 [eyeRay.y > 0] - sky
			...
			if (_FogEnabled)
			{
				half fog = saturate(1 - normalize(IN.dir).y); // fog vanishes upwards
				fog = fog / max(1, saturate((kCameraHeight)));
				col = lerp(col, unity_FogColor, fog); // blend skybox with fog
			}
