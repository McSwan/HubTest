...
fixed FoamAmount(float3 worldPos, fixed4 foam)
{
	...
	// Johan: Added Ceto_PosOffset.xz to fix foam texture not moving with camera & removed 2nd texture to be able to add mask texture.
   		foamTexture += tex2D(Ceto_FoamTexture0, (worldPos.xz + Ceto_FoamTextureScale0.z + Ceto_PosOffset.xz) * Ceto_FoamTextureScale0.xy).a;
		//foamTexture += tex2D(Ceto_FoamTexture1, (worldPos.xz + Ceto_FoamTextureScale1.z + Ceto_PosOffset.xz) * Ceto_FoamTextureScale1.xy).a * 0.5;
	#else
		foamTexture = 1.0;
	#endif
...
*/
fixed3 AboveRefractionColor(float2 grabUV, float3 surfacePos, float depth)
{
...
*/
fixed3 OceanColorFromAbove(float4 distortedUV, float3 surfacePos, float surfaceDepth)
{
	...
	fixed3 col = Ceto_DefaultOceanColor;
		...
		fixed3 refraction = AboveRefractionColor(distortedUV.zw, surfacePos, depthBlend);
